const res = [{ "bankName": "Газпромбанк", "investName": "Ваш успех", "currency": "RUB", "incomeType": 6.22, "sumMin": 50000, "sumMax": null, "termMin": 12, "termMax": 12, "canDeposit": false },
...
let userSumm = +document.getElementById("summ").value;
let userDeposit = +document.getElementById("deposit").value;
let userCurrency = +document.getElementById("currency").value;
let userTerm = +document.getElementById("term").value;

function check() {
    function originate() {
        userSumm = +document.getElementById("summ").value;
        userDeposit = +document.getElementById("deposit").value;
        userCurrency = document.getElementById("currency").value;
        userTerm = +document.getElementById("term").value;
    }
    originate();

    let deposit = new Deposit(userCurrency, userDeposit, userTerm, userSumm);

    let bankProducts = [];

    for (let i = 0; i < res.length; i++) {
        bankProducts.push(new BankProduct(res[i].bankName, res[i].investName, res[i].currency, res[i].incomeType, res[i].sumMin, res[i].sumMax, res[i].termMin, res[i].termMax, res[i].canDeposit))
    }

    bankProducts = bankProducts.filter(function (element) {
        if (element.currency === userCurrency) {
            return true
        }
    });
    ...
    bankProducts = bankProducts.filter(function (element) {
        if (userTerm >= element.termMin && userTerm <= element.termMax) {
            return true;
        } else {
            return false;
        }
    ...
    bankProducts = bankProducts.filter(function (element) {
        if ((userSumm >= element.sumMin) && (userSumm <= element.sumMax || element.sumMax === null)){
            return true;
        } else {
            return false;
        }
    ...
    for (let i = 0; i < bankProducts.length; i++) {  
        
        let total= calculator (deposit.userSumm, bankProducts[i].incomeType, deposit.userTerm, deposit.userDeposit);
    ...
    bankProducts = bankProducts.filter(function (element) {
        let total= calculator (deposit.userSumm, element.incomeType, deposit.userTerm, deposit.userDeposit);
        if ( total === maxTotal) {
            return true;
        } else {
            return false;
    ...
    function calculator (summ, interest, term, deposit) {
        for (let i = 0; i < term; i++) {            
            summ *= (1 + interest / 100 / 12);
            summ += deposit;
        }
        summ -= deposit;
        return summ;
    ...
    // for (element of bankProducts) {
    //     finalOptions.push(element.bankName, element.investName, element.incomePercent, calculateFinalSum(element, userInitSum, userTerm));
    // }
